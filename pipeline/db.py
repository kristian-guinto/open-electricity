...
from pipeline.config import DUCKDB_PATH, MOTHERDUCK_TOKEN, MOTHERDUCK_DATABASE, DB_MODE, COUNTRIES_CONFIG
...
class Database:
    ...
    def upsert_facilities(self, facilities: List[Dict[str, Any]], country_code: str = "PH") -> int:
        if not facilities:
            return 0
    ...
    def upsert_dispatch_5m(self, records: List[Dict[str, Any]], country_code: str = "PH") -> int:
        if not records:
            return 0
        ...
        data = [
            (
                ...
                float(r.get("price_local", r.get("price_php_mwh"))) if (r.get("price_local") is not None or r.get("price_php_mwh") is not None) else None,
    ...
    def upsert_regional_summary_5m(self, records: List[Dict[str, Any]], country_code: str = "PH") -> int:
        if not records:
            return 0
    ...
    def upsert_daily_stats(self, records: List[Dict[str, Any]], country_code: str = "PH") -> int:
        if not records:
            return 0
        ...
        data = [
            (
                ...
                float(r.get("avg_price_local", r.get("avg_price_php_mwh"))) if (r.get("avg_price_local") is not None or r.get("avg_price_php_mwh") is not None) else None,
                r.get("currency", currency),
    ...
    def inspect_database(
        self, country_code: Optional[str] = None, table: Optional[str] = None, region: Optional[str] = None, limit: int = 15
    ):
        ...
        for tbl, desc in tables_info:
            try:
                cnt = self.conn.execute(f"SELECT COUNT(*) FROM {tbl}{country_filter}", c_params).fetchone()[0]
                time_span = "—"
                if tbl in ("energy_dispatch_5m", "regional_summary_5m") and cnt > 0:
                    min_t, max_t = self.conn.execute(f"SELECT MIN(timestamp), MAX(timestamp) FROM {tbl}{country_filter}", c_params).fetchone()
                    if min_t and max_t:
                        time_span = f"{min_t[:10]} -> {max_t[:10]}"
                elif tbl == "energy_daily_stats" and cnt > 0:
                    min_d, max_d = self.conn.execute(f"SELECT MIN(date), MAX(date) FROM {tbl}{country_filter}", c_params).fetchone()
                    if min_d and max_d:
        ...
        if target_table in ("facilities", "facility", "all"):
            ...
            where_str = f" WHERE {' AND '.join(filter_clauses)}" if filter_clauses else ""
            ...
            print(f"  {'CT':<4} | {'ID':<14} | {'Facility Name':<30} | {'Region':<8} | {'Fuel':<10} | {'Cap(MW)':<8} | {'RE?'}")
            print(f"  {'-'*92}")
            for r in rows:
                print(f"  {r[0]:<4} | {r[1]:<14} | {r[2][:30]:<30} | {r[3]:<8} | {r[4]:<10} | {r[5] or 0:<8.1f} | {'Yes' if r[6] else 'No'}")
        ...
        if target_table in ("dispatch", "energy_dispatch_5m", "all") or not table:
            ...
            print(f"  {'CT':<4} | {'Timestamp':<24} | {'Region':<8} | {'Fuel Tech':<12} | {'Output(MW)':<11} | {'Spot Price'}")
            print(f"  {'-'*82}")
            for r in rows:
                p_curr = r[6] or "PHP"
                p_sym = COUNTRIES_CONFIG.get(r[0], {}).get("currency_symbol", "$")
                p_str = f"{p_sym}{r[5]:.2f} {p_curr}" if r[5] is not None else "—"
                print(f"  {r[0]:<4} | {r[1]:<24} | {r[2]:<8} | {r[3]:<12} | {r[4]:<11.1f} | {p_str}")
